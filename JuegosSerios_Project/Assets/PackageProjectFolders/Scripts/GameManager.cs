...
public class GameManager : MonoBehaviour
{
    ...
    public enum sceneName { None, MainMenu, Gameplay}
    ...
    public void ChangeScene(int level)
    {
        ...
        StartCoroutine(ChangeSceneCoroutine(level, sceneName.None));
    }

    float transitionsMove;
    // Se encarga de mostrar la transicion y cambiar de escena
    IEnumerator ChangeSceneCoroutine(int level, sceneName _sceneName)
    {
        ...
        {
            AudioManager_PK.GetInstance().ChangeBackgroundMusic(_sceneName);
            SceneManager.LoadScene(_sceneName.ToString());
        }
        else
        {
            if (SceneManager.GetActiveScene().name == "MainMenu_Scene")
                AudioManager_PK.GetInstance().ChangeBackgroundMusic(sceneName.Gameplay);
