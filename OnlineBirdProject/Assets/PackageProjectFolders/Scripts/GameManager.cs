...
{
    static public PlayMode playMode { get; private set; } = PlayMode.local;
    [Serializable]
    public enum PlayMode { local, online }
    static public void SetPlayMode(PlayMode newPlayMode) { playMode = newPlayMode; }

    static public bool IsLocal()
    { return playMode == PlayMode.local; }
    static public bool IsOnline()
    { return playMode == PlayMode.online; }
    ...
    #region Change Scene
    public void ChangeScene(SceneLoader.SceneName sceneName)
    {
    ...
    public void ChangeScene(int level)
    {
        ...
        StartCoroutine(ChangeSceneCoroutine(level, SceneLoader.SceneName.None));
    }

    float transitionsMove;
    // Se encarga de mostrar la transicion y cambiar de escena
    IEnumerator ChangeSceneCoroutine(int level, SceneLoader.SceneName sceneNameEnum)
    {
        ...
        if (sceneNameEnum != SceneLoader.SceneName.None)
        {
            string sceneName = sceneNameEnum.ToString();
            AudioManager_PK.GetInstance().ChangeBackgroundMusic(sceneNameEnum);
            SceneManager.LoadScene(sceneName);
        }
        else
        {
            if (SceneManager.GetActiveScene().name == "MainMenu_Scene")
                AudioManager_PK.GetInstance().ChangeBackgroundMusic(SceneLoader.SceneName.GameScene);
